      ...
      <div id="sidebar" className="visible">
        <Controls
          ...
          onChange={({timezone, date_from, date_to, breakdown, filter, noCache}) => {
            this.props.history.push(`/dmb/${formatTimezone(timezone)}/${date_from}/${date_to}/${filter.length == 0 ? '-' : filter}/${breakdown}`)
            const url = `/api/v1/dmb/${timezone}/${date_from}/${date_to}/${filter.length == 0 ? '-' : filter}/${breakdown}${noCache ? `?cache_buster=${new Date().valueOf()}` : ''}`
