  ...
  return <TABLE width={1060}>
    <thead>
      <tr>
        ...
        <TH width={90} value={ show_label_row('Pixels', 'pixels') } onClick={ () => onSort('row', 'pixels', 1) }/>
        ...
        <TH width={90} value={ show_label_row('Active%', 'active') } onClick={ () => onSort('row', 'active', 1) }/>
        <TH width={90} value={ show_label_row('Cost', 'cost') } onClick={ () => onSort('row', 'cost', 1) }/>
        <TH width={90} value={ show_label_row('CPA', 'home_cpa') } onClick={ () => onSort('row', 'home_cpa', 1) }/>
      </tr>
    </thead>
    <tbody>{
      data.data.map((x, i) => <tr key={i}>
        <TD width={150}  value={ formatter(params.section)(x.section) } style={{ paddingLeft: '0.7em' }}  />
        <TD width={params.row == 'hour' ? 180 : 150} value={ formatter(params.row)(x.row) } />
        ...
        <TD width={90}  value={d3.format(',')(+x.pixels) + change_sign(+x.pixels_change)} />
        <TD width={90}  value={d3.format('0.0f')(100 * +x.pixels_ratio) + change_sign(+x.pixels_ratio_change)} />
        <TD width={90}  value={d3.format('0.2f')(+x.ecpa) + change_sign(+x.ecpa_change)} />
        ...
        <TD width={90}  value={d3.format('0.0f')(100 * +x.active) + change_sign(+x.active_change * 0.5)} />
        <TD width={90}  value={d3.format(',.0f')(+x.cost) + change_sign(+x.cost_change)} />
        <TD width={90}  value={d3.format(',.2f')(+x.home_cpa) + change_sign(+x.home_cpa_change)} />
      </tr>)
    }
      <tr>
        ...
        <TD width='90'  style={ { fontWeight: 'bold' } } value={d3.format(',')(+data.pixels) + change_sign(+data.pixels_change)} />
        <TD width={90}  style={ { fontWeight: 'bold' } } value={d3.format('0.0f')(100 * +data.pixels_ratio) + change_sign(+data.pixels_ratio_change)} />
        <TD width={90}  style={ { fontWeight: 'bold' } } value={d3.format('0.2f')(+data.ecpa) + change_sign(+data.ecpa_change)} />
        ...
        <TD width={90}  style={ { fontWeight: 'bold' } } value={d3.format('0.0f')(100 * +data.active) + change_sign(+data.active_change * 0.5)} />
        <TD width={90}  style={ { fontWeight: 'bold' } } value={d3.format(',.0f')(+data.cost) + change_sign(+data.cost_change)} />
        <TD width={90}  style={ { fontWeight: 'bold' } } value={d3.format(',.2f')(+data.home_cpa) + change_sign(+data.home_cpa_change)} />
      </tr>
    </tbody>
  </TABLE>
}
